...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-secondary-900 flex items-center gap-2">
                <ion-icon name="person-add" class="text-primary-600 text-2xl"></ion-icon>
                নতুন কর্মচারী যোগ করুন (Add New Employee)
            </h2>
            <p class="text-secondary-500 text-sm mt-1">কর্মচারীর ব্যক্তিগত, অফিসিয়াল, বেতন ও ব্যাংকিং তথ্য পূরণ করুন</p>
        </div>
        <a href="<?= $base ?>/admin/hr/employees" class="px-4 py-2 bg-secondary-100 hover:bg-secondary-200 text-secondary-700 rounded-xl text-sm font-medium flex items-center gap-1.5 transition-colors">
            ...
            <span>কর্মচারী তালিকায় ফিরুন</span>
        </a>
    </div>
    ...
    <form method="POST" action="<?= $base ?>/admin/hr/employees/store" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl border border-secondary-200 shadow-sm p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="nid" placeholder="NID নম্বর" class="w-full bg-secondary-50 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm text-secondary-800 focus:ring-2 focus:ring-primary-500">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="emergency_contact" placeholder="আত্মীয় / অভিভাবকের নম্বর" class="w-full bg-secondary-50 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm text-secondary-800 focus:ring-2 focus:ring-primary-500">
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col items-center justify-center p-4 border-2 border-dashed border-secondary-300 rounded-2xl bg-secondary-50">
                    ...
                    <p class="text-[11px] text-secondary-400 mt-2 text-center">JPG, PNG বা WEBP (ম্যাক্স ২ এমবি)</p>
                </div>
            </div>

            <div>
                <label class="block text-xs font-semibold text-secondary-700 uppercase tracking-wider mb-1">বর্তমান ও স্থায়ী ঠিকানা</label>
                <textarea name="address" rows="2" placeholder="বাসা/গ্রাম, ডাকঘর, থানা, জেলা..." class="w-full bg-secondary-50 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm text-secondary-800 focus:ring-2 focus:ring-primary-500"></textarea>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-secondary-200 shadow-sm p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-xs font-semibold text-secondary-700 uppercase tracking-wider mb-1">কর্মচারী কোড (EMP Code) <span class="text-red-500">*</span></label>
                    <input type="text" name="emp_code" value="<?= htmlspecialchars($nextEmpCode) ?>" required class="w-full bg-secondary-100 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm font-mono font-bold text-secondary-800 focus:ring-2 focus:ring-primary-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-secondary-700 uppercase tracking-wider mb-1">যোগদানের তারিখ <span class="text-red-500">*</span></label>
                    <input type="date" name="joining_date" value="<?= date('Y-m-d') ?>" required class="w-full bg-secondary-50 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm text-secondary-800 focus:ring-2 focus:ring-primary-500">
                </div>
                <div>
                    ...
                    <select name="employment_type" class="w-full bg-secondary-50 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm text-secondary-800 focus:ring-2 focus:ring-primary-500">
                        ...
                        <option value="daily">দৈনিক হাজিরা (Daily Basis)</option>
                    </select>
                </div>
            </div>
                ...
                <div>
                    ...
                    <select name="designation_id" id="designationSelect" class="w-full bg-secondary-50 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm text-secondary-800 focus:ring-2 focus:ring-primary-500">
                        ...
                        <?php foreach ($designations as $des): ?>
                            <option value="<?= $des['id'] ?>" data-dept="<?= $des['department_id'] ?>"><?= htmlspecialchars($des['title'] . ' (' . $des['department_name'] . ')') ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div>
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-4">
                    ...
                    <label class="inline-flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="status" value="on_leave" class="text-primary-600 focus:ring-primary-500">
        ...
        <div class="bg-white rounded-2xl border border-secondary-200 shadow-sm p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                    ...
                    <input type="number" step="0.01" name="basic_salary" id="basicSalary" value="0.00" oninput="calculateGross()" required class="w-full bg-secondary-50 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm font-bold text-secondary-800 focus:ring-2 focus:ring-primary-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-secondary-700 uppercase tracking-wider mb-1">বাড়ি ভাড়া (House Rent)</label>
                    <input type="number" step="0.01" name="house_rent" id="houseRent" value="0.00" oninput="calculateGross()" class="w-full bg-secondary-50 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm text-secondary-800 focus:ring-2 focus:ring-primary-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-secondary-700 uppercase tracking-wider mb-1">চিকিৎসা ভাতা (Medical)</label>
                    <input type="number" step="0.01" name="medical_allowance" id="medicalAllowance" value="0.00" oninput="calculateGross()" class="w-full bg-secondary-50 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm text-secondary-800 focus:ring-2 focus:ring-primary-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-secondary-700 uppercase tracking-wider mb-1">অন্যান্য ভাতা (Other)</label>
                    <input type="number" step="0.01" name="other_allowance" id="otherAllowance" value="0.00" oninput="calculateGross()" class="w-full bg-secondary-50 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm text-secondary-800 focus:ring-2 focus:ring-primary-500">
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-secondary-200 shadow-sm p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="bank_name" placeholder="e.g. Dutch-Bangla Bank, Islami Bank" class="w-full bg-secondary-50 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm text-secondary-800 focus:ring-2 focus:ring-primary-500">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-secondary-700 uppercase tracking-wider mb-1">ব্যাংক একাউন্ট নম্বর</label>
                    <input type="text" name="bank_account_no" placeholder="হিসাব নম্বর" class="w-full bg-secondary-50 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm text-secondary-800 focus:ring-2 focus:ring-primary-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="mobile_banking_number" placeholder="01xxxxxxxxx" class="w-full bg-secondary-50 border border-secondary-300 rounded-xl px-3.5 py-2 text-sm text-secondary-800 focus:ring-2 focus:ring-primary-500">
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-4">
            <a href="<?= $base ?>/admin/hr/employees" class="px-6 py-2.5 border border-secondary-300 text-secondary-700 rounded-xl text-sm font-medium hover:bg-secondary-50 transition-colors">বাতিল</a>
            <button type="submit" class="px-8 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl text-sm font-medium transition-colors shadow-md flex items-center gap-2">
                ...
                <span>কর্মচারী সংরক্ষণ করুন</span>
            </button>
        </div>
    </form>
</div>
...
function filterDesignations(deptId) {
