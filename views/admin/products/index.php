    ...
    <input type="hidden" name="csrf_token" value="<?= \Core\CSRF::token() ?>">
                ...
                <div class="mb-4 bg-secondary-50/70 p-3.5 rounded-xl border border-secondary-200">
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <span class="text-[10px] text-secondary-400 font-medium">ধাপে ধাপে সিলেক্ট হবে</span>
                    </div>
...
// --- CASCADING CATEGORY SELECTOR CLASS (for Single Add) ---
class CascadingCategorySelector {
    ...
    updateBreadcrumb() {
        ...
        if (!breadcrumbEl) {
            ...
            breadcrumbEl.className = 'category-breadcrumb mt-2 text-xs px-3 py-1.5 rounded-xl flex items-center gap-1.5';
        ...
        if (this.selectedId) {
            ...
            if (hasChildren) {
                breadcrumbEl.innerHTML = `<ion-icon name="arrow-forward-circle-outline" class="text-base text-amber-600 flex-shrink-0"></ion-icon> <div><span class="text-amber-800 font-bold">সাব-ক্যাটাগরি সিলেক্ট করুন:</span> <span class="text-secondary-800">${pathNames}</span></div>`;
                ...
                breadcrumbEl.innerHTML = `<ion-icon name="checkmark-circle" class="text-base text-emerald-600 flex-shrink-0"></ion-icon> <div><span class="font-bold text-emerald-900">নির্বাচিত:</span> <span class="text-secondary-900 font-semibold">${pathNames}</span></div>`;
                breadcrumbEl.className = 'category-breadcrumb mt-2 text-xs font-semibold text-emerald-800 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-xl flex items-center gap-1.5';
            }
