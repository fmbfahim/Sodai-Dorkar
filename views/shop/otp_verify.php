  ...
  <div class="w-full max-w-md fade-in">
    ...
    <div class="bg-white rounded-3xl shadow-2xl border border-emerald-100 overflow-hidden">
      ...
      <div class="p-8 sm:p-10">
        ...
        <!-- DEV HELPER: show OTP from session -->
        <?php
        if (session_status() === PHP_SESSION_NONE) session_start();
        $devOtp = $_SESSION['pending_otp'] ?? null;
        if (!$devOtp && isset($_SESSION['customer_id'])) {
            $devStmt = (new \Core\Database(require __DIR__ . '/../../../config/database.php'))->query(
                "SELECT otp_code FROM customers WHERE id = ?", [$_SESSION['customer_id']]
            );
            $devRow = $devStmt->fetch();
            $devOtp = $devRow['otp_code'] ?? null;
        }
        if ($devOtp): ?>
